...
public class ChallengeThree {
	public static List<String> topSalaryReport(List<Employee> employees, int top, Formattable formatter) {
		...
		employees.stream().sorted((e1, e2) -> Integer.compare(e2.salary, e1.salary)).limit(top).forEach((el) -> topEmployees.add(
				formatter.format(el.name)));
		return topEmployees;
		// ====================================
	...
	public static void main(String[] args) {
		...
		employees.add(new Employee("Judy Diaz", 90000));
		employees.add(new Employee("Mark Bourne", 85000));
		topSalaryReport(employees, 3, String::toUpperCase).forEach(System.out::println);
		// Expected output is
		// JUDY DIAZ
		// MARK BOURNE
	}
